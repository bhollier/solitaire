...
fn test_take_n() {
    let d: std::Deck = std::Card::new_deck();
    {
        let (rest, cs) = take_n_slice(d.as_slice(), 10);
        assert_eq!(rest, &d[..42]);
        assert_eq!(cs, &d[42..52]);
    }

    {
        let (rest, cs) = take_n_vec(&d.iter().collect(), 10);
...
fn test_take_one() {
    let d: std::Deck = std::Card::new_deck();
    {
        let (rest, c) = take_one_slice(d.as_slice());
        assert_eq!(rest, &d[..51]);
        assert_eq!(c, &d[51]);
    }

    {
        let (rest, c) = take_one_vec(&d.iter().collect());
